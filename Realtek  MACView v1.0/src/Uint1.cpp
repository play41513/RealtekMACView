...
void __fastcall TUSBHIDForm::FormClose(TObject *Sender, TCloseAction &Action) {
	...
	if(bFirstTest)
	{
		...
		if(LOGFilePath_2!="")
			DeleteFile(LOGFilePath_2+"\\"+LOGFile);
	}
...
{
	AnsiString path = LOGFilePath_2+LOGFile;
	if(FileExists(path.c_str()))
		ImgLogPath->Picture->LoadFromFile("Image\\Green.bmp");
	else ImgLogPath->Picture->LoadFromFile("Image\\Gray.bmp");
...
bool TUSBHIDForm::writeLOG(AnsiString Msg,bool bAction) {
	...
	{   //創LOG資料夾
		...
		if(plLOG_Path->Caption !="")
		{
			...
			SaveLogLine(LOGFilePath_2+LOGFile, Msg);
		}
...
{
	AnsiString Resultdata = DosCommand("devcon find \"\*usb\\vid_0bda\*\"");
...
void __fastcall TUSBHIDForm::btnSetClick(TObject *Sender)
{
	if(btnSet->Caption == "Enter")
	{
		...
		{
			edtPassWord->Text ="";
			plCharacteristics->Height = plCharacteristics->Height==401?71:401;
		}
...
void TUSBHIDForm::SetUIEnabled(bool bEnabled,bool bSet)
{
	...
	else
	{
		...
		plLOG_Path->PopupMenu 		= bEnabled ? PopupMenuLOG:NULL;
	}
}
//---------------------------------------------------------------------------
