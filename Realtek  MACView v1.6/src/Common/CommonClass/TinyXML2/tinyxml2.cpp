...
char* XMLDocument::Identify( char* p, XMLNode** node )
{
    ...
    if ( XMLUtil::StringEqual( p, xmlHeader, xmlHeaderLen ) ) {
		TIXMLASSERT( sizeof( XMLDeclaration ) == (size_t)_commentPool.ItemSize() );
        returnNode = new (_commentPool.Alloc()) XMLDeclaration( this );
        returnNode->_memPool = &_commentPool;
        p += xmlHeaderLen;
    }
    else if ( XMLUtil::StringEqual( p, commentHeader, commentHeaderLen ) ) {
		TIXMLASSERT( sizeof( XMLComment ) == (size_t)_commentPool.ItemSize() );
        returnNode = new (_commentPool.Alloc()) XMLComment( this );
        returnNode->_memPool = &_commentPool;
        p += commentHeaderLen;
    }
    else if ( XMLUtil::StringEqual( p, cdataHeader, cdataHeaderLen ) ) {
		TIXMLASSERT( sizeof( XMLText ) == (size_t)_textPool.ItemSize() );
		...
		TIXMLASSERT( sizeof( XMLUnknown ) == (size_t)_commentPool.ItemSize() );
        returnNode = new (_commentPool.Alloc()) XMLUnknown( this );
        returnNode->_memPool = &_commentPool;
        p += dtdHeaderLen;
    }
    else if ( XMLUtil::StringEqual( p, elementHeader, elementHeaderLen ) ) {
		TIXMLASSERT( sizeof( XMLElement ) == (size_t)_elementPool.ItemSize() );
...
XMLAttribute* XMLElement::FindOrCreateAttribute( const char* name )
{
    ...
    if ( !attrib ) {
		TIXMLASSERT( sizeof( XMLAttribute ) == (size_t)_document->_attributePool.ItemSize() );
